...
interface PrivacySocialData {
  privateProfile: boolean;
  hideSuggested: boolean;
  defaultWorkoutVisibility: 'public' | 'friends' | 'private';
}

export const PrivacySocial = () => {
  const navigate = useNavigate();
  const { toast } = useToast();
  
  const [formData, setFormData] = useState<PrivacySocialData>({
    privateProfile: false,
    hideSuggested: false,
    defaultWorkoutVisibility: 'private'
  });
  const [originalData, setOriginalData] = useState<PrivacySocialData>({
    privateProfile: false,
    hideSuggested: false,
    defaultWorkoutVisibility: 'private'
  });
