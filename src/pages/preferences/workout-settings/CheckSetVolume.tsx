...
export const CheckSetVolume = () => {
  const navigate = useNavigate();
  const [selectedVolume, setSelectedVolume] = useState("Off");

  useEffect(() => {
    const saved = localStorage.getItem('soundSettings');
    if (saved) {
      const settings = JSON.parse(saved);
      setSelectedVolume(settings.checkSet || "Off");
    }
  }, []);
  ...
  const handleSelect = (volume: string) => {
    ...
    settings.checkSet = volume;
    localStorage.setItem('soundSettings', JSON.stringify(settings));
    toast.success("Check set volume updated");
  };
      ...
      <div className="sticky top-0 z-50 bg-background border-b">
        <div className="flex items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold">Check Set Volume</h1>
          </div>
        </div>
      </div>
